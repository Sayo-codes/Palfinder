...
const TYPE_STYLES: Record<string, { bg: string; color: string; label: string }> = {
  ...
  other:        { bg: 'var(--border)', color: 'var(--text-muted)', label: 'Other' },
}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-bold text-foreground">Input Logs</h1>
      ...
      {/* Table card */}
      <div className="rounded-2xl overflow-hidden bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10">
        {logs.length === 0 ? (

          /* Empty state */
          <div className="flex flex-col items-center justify-center py-20 gap-3">
            <div className="w-14 h-14 rounded-2xl flex items-center justify-center bg-black/5 dark:bg-white/5">
              <ClipboardListIcon className="w-7 h-7 text-foreground/20" />
            </div>
            <p className="text-sm font-semibold text-foreground/40">No inputs yet</p>
              ...
              <thead>
                <tr className="border-b border-border">
                  {['Time', 'Value', 'Page', 'Type'].map(h => (
                    <th
                      key={h}
                      className="text-left px-5 py-3 text-[11px] font-semibold uppercase tracking-wider text-foreground/35"
                    >
                      {h}
                    </th>
                  ))}
                ...
                {logs.map((log: any, i: number) => (
                  ...
                  >
                    ...
                    <td className="px-5 py-3.5">
                      <span className="text-foreground/40 font-mono text-xs truncate block max-w-[200px]" title={log.page}>
                        {log.page}
                      </span>
                    </td>
