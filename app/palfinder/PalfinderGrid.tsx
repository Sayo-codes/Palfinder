  ...
  return (
    <div ref={backdropRef}
      ...
      <div className="relative w-full max-w-2xl max-h-[90vh] rounded-2xl overflow-hidden flex flex-col"
        style={{ background: 'var(--surface)', border: '1px solid rgba(255,255,255,0.08)', boxShadow: '0 24px 80px rgba(0,0,0,0.6)' }}>
        ...
        <div className="px-5 py-4 overflow-y-auto flex-1" style={{ maxHeight: '35vh' }}>
          <div className="flex items-start justify-between gap-4 mb-3">
            <div>
              ...
              <p className="text-xs text-white/40 mt-0.5">{profile.location} · {profile.age} years old</p>
            </div>
            <div className="text-right flex-shrink-0">
              <div className="flex items-center gap-0.5 justify-end mb-1">
                {[1,2,3,4,5].map(s => (
                  <StarIcon key={s} className={`w-3.5 h-3.5 ${s <= profile.rating ? 'fill-palfinder-gold text-palfinder-gold' : 'fill-white/10 text-white/10'}`} />
                ))}
              </div>
          ...
          <p className="text-sm text-white/55 italic leading-relaxed mb-4">&ldquo;{profile.bio}&rdquo;</p>
...
function ProfileCard({
  ...
  return (
    <div className="group rounded-2xl overflow-hidden flex flex-col transition-all duration-300 hover:-translate-y-1 text-left cursor-pointer"
      ...
      <div className="w-full aspect-[4/5] relative overflow-hidden">
        {profile.mainPhoto
          ...
          : <div className="w-full h-full bg-white/5 flex items-center justify-center"><ImageIcon className="w-10 h-10 text-white/20" /></div>}
      ...
      <div className="p-3.5 flex flex-col flex-grow">
        <div className="mb-1">
          ...
          {/* Location + Age */}
          <p className="text-[11px] text-white/40 mt-0.5">{profile.location} · <span className="text-white/50">{profile.age} yrs</span></p>
        </div>

        <p className="text-[11px] text-white/50 italic leading-snug mb-2.5 line-clamp-2">&ldquo;{profile.bio}&rdquo;</p>
    ...
    <div className="min-h-screen w-full">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pt-6 pb-16">
        ...
        <div className="flex justify-between items-center mb-10">
          <Link href="/" className="flex items-center gap-1 text-white/70 hover:text-white text-sm rounded-md transition-colors">
        ...
        <div className="text-center mb-10">
          <div className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full text-xs font-semibold mb-5"
            style={{ background: 'rgba(107,31,42,0.12)', color: '#E8B547', border: '1px solid rgba(107,31,42,0.3)' }}>
          ...
          <h1 className="text-4xl sm:text-5xl lg:text-6xl font-extrabold leading-[1.1] tracking-tight mb-4">
            ...
            <span className="bg-clip-text text-transparent" style={{ backgroundImage: 'linear-gradient(135deg, #6B1F2A, #E8B547)' }}>
              Perfect Match
            </span>
          </h1>
        ...
        <div className="flex gap-2.5 overflow-x-auto no-scrollbar pb-1 mb-8 justify-center">
          {['Prices ($)', 'Age Range'].map(label => (
            <button key={label} className="flex items-center gap-1.5 rounded-full py-1.5 px-4 text-sm font-medium whitespace-nowrap shrink-0 transition-all hover:scale-105"
              style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.09)', color: 'rgba(255,255,255,0.6)' }}>
              {label} <ChevronDownIcon className="w-3.5 h-3.5 text-white/40" strokeWidth={2} />
            </button>
          ))}
        </div>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-lg font-extrabold">
                <span className="text-white">Pal</span><span style={{ color: '#6B1F2A' }}>finder</span>
              </span>
            </div>
