...
interface DBProfile {
  ...
  mainPhoto: string; gallery: string[]
}
...
export default function PaymentClient({ profile }: { profile: DBProfile }) {
  ...
  /* Auto-create when profile loads */
  useEffect(() => {
    if (profile) createPayment(currency)
  ...
  return (
    <div className="min-h-screen" style={{ background: 'linear-gradient(160deg,#08080F 0%,#0D0A14 60%,#0A0510 100%)' }}>
      ...
      <div className="fixed inset-0 pointer-events-none overflow-hidden" aria-hidden>
        <div style={{ position:'absolute', top:'-20%', left:'50%', transform:'translateX(-50%)', width:'600px', height:'600px', borderRadius:'50%', background:'radial-gradient(circle,rgba(107,31,42,0.12) 0%,transparent 70%)', filter:'blur(40px)' }}/>
      </div>
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="grid lg:grid-cols-[1fr_420px] gap-8 items-start">
          ...
          {/* ── Right: Payment panel ── */}
          <div className="rounded-2xl overflow-visible" style={{ background:'linear-gradient(180deg,#0D0D1A 0%,#0A0A14 100%)', border:'1px solid rgba(255,255,255,0.08)', boxShadow:'0 24px 80px rgba(0,0,0,0.6),0 0 40px rgba(107,31,42,0.1)' }}>

            {/* Header */}
            <div className="flex items-center gap-3 px-5 py-4" style={{ borderBottom:'1px solid rgba(255,255,255,0.06)' }}>
              <div className="w-9 h-9 rounded-full flex items-center justify-center flex-shrink-0" style={{ background:'linear-gradient(135deg,#6B1F2A,#E8B547)', boxShadow:'0 0 16px rgba(107,31,42,0.5)' }}>
            ...
            <div className="px-5 py-5 space-y-5">
              ...
              <div className="flex justify-center">
                <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full text-xs font-semibold" style={{ background: sm.bg, color: sm.color, border:`1px solid ${sm.color}22` }}>
              ...
              <div className="relative">
                ...
                <button onClick={() => setShowDrop(v => !v)} disabled={isActive || status === 'loading'}
                ...
                {showDrop && (
                  <div className="absolute top-full left-0 right-0 mt-1 rounded-xl overflow-hidden z-20 max-h-52 overflow-y-auto" style={{ background:'#111122', border:'1px solid rgba(255,255,255,0.1)', boxShadow:'0 8px 40px rgba(0,0,0,0.7)' }}>
                    {CURRENCIES.map(c => (
              ...
              {status === 'error' && (
                <div className="text-center py-6 space-y-4">
                  ...
                  <button onClick={retry} className="px-6 py-2 rounded-full text-sm font-bold text-white transition hover:brightness-110" style={{ background:'#6B1F2A', boxShadow:'0 4px 20px rgba(107,31,42,0.4)' }}>Try Again</button>
                </div>
              )}
              ...
              {payment && isActive && (
                <>
                  <div className="rounded-xl p-4" style={{ background:'rgba(255,255,255,0.03)', border:'1px solid rgba(255,255,255,0.07)' }}>
                    ...
                    <p className="text-2xl font-bold text-white">{payment.payAmount}{' '}<span className="text-sm font-medium text-white/50 uppercase">{payment.payCurrency}</span></p>
                  </div>
                  <div className="flex flex-col items-center gap-3">
                    ...
                    <div className="flex items-center gap-1.5 text-[10px] text-white/30"><QrCodeIcon className="w-3 h-3" /> Scan with your crypto wallet</div>
                  </div>
                  <div>
                    <label className="block text-[11px] font-medium text-white/40 mb-1.5 uppercase tracking-wider">Wallet Address</label>
                    <div className="flex items-center gap-2 rounded-xl px-3 py-3" style={{ background:'rgba(255,255,255,0.03)', border:'1px solid rgba(255,255,255,0.07)' }}>
                      <p className="flex-1 text-xs text-white/80 font-mono break-all leading-relaxed select-all">{payment.payAddress}</p>
                      <button onClick={handleCopy} className="flex-shrink-0 w-8 h-8 rounded-lg flex items-center justify-center transition" style={{ background: copied ? 'rgba(0,209,104,0.15)' : 'rgba(255,255,255,0.06)' }}>
                  ...
                  <div className="flex items-center justify-center gap-2 py-1">
                    ...
                    <span className="text-lg font-mono font-bold" style={{ color: timeLeft < 300 ? '#FF4444' : timeLeft < 600 ? '#E8B547' : 'rgba(255,255,255,0.7)' }}>{fmt(timeLeft)}</span>
              ...
              {isDone && (
                <div className="flex flex-col items-center py-8 gap-5">
                  <div className="w-20 h-20 rounded-full flex items-center justify-center" style={{ background:'rgba(0,209,104,0.1)', border:'2px solid rgba(0,209,104,0.3)', boxShadow:'0 0 40px rgba(0,209,104,0.2)' }}>
                  ...
                  <button onClick={() => router.push('/palfinder')} className="mt-2 px-8 py-3 rounded-full text-sm font-bold text-white transition hover:brightness-110 active:scale-95" style={{ background:'linear-gradient(135deg,#6B1F2A,#8B2535)', boxShadow:'0 4px 20px rgba(107,31,42,0.5)' }}>
                    Continue Browsing
                  </button>
                </div>
              )}
              ...
              {status === 'expired' && (
                <div className="flex flex-col items-center py-8 gap-5">
                  ...
                  <button onClick={retry} className="px-8 py-2.5 rounded-full text-sm font-bold text-white transition hover:brightness-110" style={{ background:'#6B1F2A', boxShadow:'0 4px 20px rgba(107,31,42,0.5)' }}>
