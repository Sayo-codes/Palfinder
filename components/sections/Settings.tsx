...
function SectionCard({ title, icon, children }: { title: string; icon: React.ReactNode; children: React.ReactNode }) {
  return (
    <div className="card overflow-hidden">
      <div className="px-4 sm:px-5 py-3.5 border-b border-white/5 flex items-center gap-2.5">
  ...
  return (
    <div className="space-y-4 animate-in max-w-2xl">
      ...
      <SectionCard title="Site Settings" icon={<Globe size={16} />}>
        <div>
          <label className="block text-xs font-semibold text-white/40 uppercase tracking-wider mb-1.5">Site Name</label>
          <input className="input-base" value={siteName} onChange={(e) => setSiteName(e.target.value)} />
        </div>
        <div>
          <label className="block text-xs font-semibold text-white/40 uppercase tracking-wider mb-1.5">Site URL</label>
      ...
      <SectionCard title="Admin Account" icon={<User size={16} />}>
        <div>
          <label className="block text-xs font-semibold text-white/40 uppercase tracking-wider mb-1.5">Admin Email</label>
