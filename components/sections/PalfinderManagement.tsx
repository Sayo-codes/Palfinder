...
export default function PalfinderManagement() {
  ...
  /* ── Render ─────────────────────────────────────────────── */
  const inputCls = 'w-full bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-xl px-4 py-2.5 text-foreground placeholder:text-foreground/30 focus:outline-none focus:border-[#FF1B8D] transition'
    ...
    <div className="space-y-6 animate-in">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-foreground/40 text-sm mt-1">Profiles saved here appear live on the public PalFinder page.</p>
        </div>
      ...
      {/* Table */}
      <div className="bg-palfinder-surface border border-border rounded-2xl overflow-hidden shadow-xl">
        {loading ? (
          <div className="flex items-center justify-center py-16 gap-3">
            <Loader2 className="w-6 h-6 text-[#FF1B8D] animate-spin" />
            <span className="text-foreground/40 text-sm">Loading profiles…</span>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm whitespace-nowrap">
              <thead className="bg-black/5 dark:bg-white/5 text-foreground/50 text-xs uppercase font-medium">
                <tr>
                  ...
                  <tr key={p.id} className="hover:bg-black/[0.02] dark:hover:bg-white/[0.02] transition-colors">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-full overflow-hidden bg-black/5 dark:bg-white/10 flex-shrink-0 relative">
                          {p.mainPhoto
                            ? <img src={p.mainPhoto} alt={p.name} className="w-full h-full object-cover" />
                            : <ImageIcon className="w-5 h-5 absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-foreground/30" />}
                        </div>
                        <div>
                          <div className="text-foreground font-semibold">{p.name}</div>
                          <div className="text-foreground/30 text-xs font-mono">{p.id.slice(0, 12)}…</div>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4 text-foreground/70">{p.location}</td>
                    <td className="px-6 py-4 text-foreground/70">{p.age} yrs</td>
                    <td className="px-6 py-4 font-semibold text-[#FF1B8D]">${p.price}</td>
                    <td className="px-6 py-4 text-foreground/70">⭐ {p.rating.toFixed(1)}</td>
                    <td className="px-6 py-4">
                      <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold uppercase ${p.status === 'active' ? 'bg-[#00FF7F]/10 text-[#00FF7F] border border-[#00FF7F]/20' : 'bg-black/10 dark:bg-white/10 text-foreground/50 border border-border'}`}>
                        {p.status}
                      </span>
                    </td>
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <button onClick={() => openModal(p)} className="w-8 h-8 rounded-lg flex items-center justify-center bg-black/5 dark:bg-white/5 hover:bg-black/10 dark:hover:bg-white/10 text-foreground/60 hover:text-foreground transition"><Edit2 size={14} /></button>
                        <button onClick={() => handleDelete(p.id)} className="w-8 h-8 rounded-lg flex items-center justify-center bg-black/5 dark:bg-white/5 hover:bg-red-500/20 text-foreground/60 hover:text-red-400 transition"><Trash2 size={14} /></button>
                      </div>
                    </td>
                  </tr>
                ))}
                {!profiles.length && (
                  <tr><td colSpan={7} className="px-6 py-14 text-center text-foreground/30">No profiles yet — click &quot;Add New Profile&quot; to get started.</td></tr>
                )}
              </tbody>
            </table>
          </div>
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 backdrop-blur-md bg-black/60">
          <div className="bg-palfinder-surface border border-border rounded-2xl w-full max-w-4xl max-h-[90vh] flex flex-col shadow-2xl overflow-hidden">

            {/* Modal header */}
            <div className="flex items-center justify-between px-6 py-4 border-b border-border bg-palfinder-surface2">
              <h2 className="text-lg font-bold text-foreground">{editingId ? 'Edit Profile' : 'New PalFinder Profile'}</h2>
              <button onClick={() => setIsOpen(false)} className="text-foreground/40 hover:text-foreground transition"><X size={20} /></button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="space-y-4">
                  <h3 className="text-xs font-semibold text-foreground/40 uppercase tracking-wider border-b border-border pb-2">Basic Info</h3>

                  <div>
                    <label className="block text-sm font-medium text-foreground/70 mb-1">Full Name <span className="text-[#FF1B8D]">*</span></label>
                  ...
                  <div>
                    <label className="block text-sm font-medium text-foreground/70 mb-1">Location <span className="text-[#FF1B8D]">*</span></label>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="block text-sm font-medium text-foreground/70 mb-1">Price (USD) <span className="text-[#FF1B8D]">*</span></label>
                      <input type="number" min="1" value={form.price} onChange={e => setForm(f => ({ ...f, price: +e.target.value }))} className={inputCls} />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-foreground/70 mb-1">Age (18–80) <span className="text-[#FF1B8D]">*</span></label>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="block text-sm font-medium text-foreground/70 mb-1">Rating (1–5)</label>
                    ...
                    <div>
                      ...
                      <select value={form.status} onChange={e => setForm(f => ({ ...f, status: e.target.value as 'active' | 'inactive' }))}
                        className="w-full bg-palfinder-surface border border-border rounded-xl px-4 py-2.5 text-foreground focus:outline-none focus:border-[#FF1B8D] transition appearance-none">
                  ...
                  <div>
                    ...
                    <div className="p-3 bg-black/5 dark:bg-white/5 border border-border rounded-xl flex flex-wrap gap-2 min-h-[48px]">
                      {form.tags.map(t => (
                      ...
                      <input type="text" value={tagInput} onChange={e => setTagInput(e.target.value)} onKeyDown={addTag}
                        className="bg-transparent text-foreground text-sm focus:outline-none flex-1 min-w-[100px]" placeholder="Type & Enter…" />
                    </div>
                  </div>
                </div>
                ...
                {/* Right — Media */}
                <div className="space-y-4">
                  <h3 className="text-xs font-semibold text-foreground/40 uppercase tracking-wider border-b border-border pb-2">Media</h3>

                  {/* Main photo */}
                  <div>
                    <label className="block text-sm font-medium text-foreground/70 mb-2">Main Profile Photo</label>
                    <div onClick={() => mainRef.current?.click()}
                      className="relative w-full h-52 rounded-xl border-2 border-dashed border-border bg-black/5 dark:bg-white/5 flex flex-col items-center justify-center cursor-pointer hover:border-[#FF1B8D]/50 hover:bg-black/10 dark:hover:bg-white/10 transition overflow-hidden group">
                      {form.mainPhoto ? (
                        <>
                          ...
                          {mainUploading
                            ...
                            : <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition flex items-center justify-center"><span className="text-foreground flex items-center gap-2 text-sm font-medium"><Upload size={16}/> Change</span></div>
                          }
                        </>
                      ) : (
                        <><Upload size={30} className="text-foreground/20 mb-2" /><span className="text-sm text-foreground/40">Click to upload</span></>
                      )}
                    </div>
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-2">
                      <label className="text-sm font-medium text-foreground/70">Gallery</label>
                      <button onClick={() => galleryRef.current?.click()} className="text-xs font-semibold text-[#FF1B8D] hover:text-[#FF1B8D]/80 transition flex items-center gap-1">
                        <Plus size={13}/> Add
                      </button>
                    </div>
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      {form.gallery.map((item, i) => (
                        <div key={item.id} draggable={!item.uploading}
                          ...
                          className={`group relative aspect-[3/4] rounded-xl overflow-hidden bg-black/5 dark:bg-white/5 border ${dragIdx === i ? 'border-[#FF1B8D] opacity-50' : 'border-border'} transition-all`}>
                          {item.type === 'video'
                      ...
                      {!form.gallery.length && (
                        <div className="col-span-3 py-8 text-center border border-dashed border-border rounded-xl text-foreground/30 text-sm">
                          No media — click &quot;Add&quot; to upload photos &amp; videos
                        </div>
                      )}
                    </div>
                    <p className="text-xs text-foreground/25 mt-2 text-center">Drag to reorder</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            {/* Modal footer */}
            <div className="px-6 py-4 border-t border-border bg-palfinder-surface2 flex justify-end gap-3">
              <button onClick={() => setIsOpen(false)} className="px-5 py-2.5 rounded-xl text-sm font-semibold text-foreground/60 hover:text-foreground hover:bg-black/5 dark:hover:bg-white/5 transition">Cancel</button>
